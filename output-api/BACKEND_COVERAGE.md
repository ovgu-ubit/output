...
- Outcome: 9/9 suites passed, producing 5.85% statements, 2.62% branches, 3.66% functions, and 6.37% lines covered.
- Coverage concentrates on service specs under `src/author`, `src/funder`, `src/contract`, `src/pub_type`, `src/oa_category`, `src/institute`, `src/publisher`, and `src/greater_entity`, with the highest numbers around 60% statements/65% lines (`author.service.ts`). Most other modules—including publication core workflows, authorization, exports, imports, and statistics—report 0% coverage.
...
## Gaps observed
1. **Publication domain lacks tests**: `src/publication/core/publication.service.ts` (>700 lines) has ~12% statements/13% lines, and related lookup/relation services have 0% coverage.
...
3. **Shared infrastructure uncovered**: Common utilities (`abstract-entity.service`, `alias-lookup.service`), config services, and authorization/token services are untested.
...
2. **Add focused unit tests for critical domains**
   ...
   - Extend tests for `authorization.service.ts` and `token.authorization.service.ts` to verify JWT/token issuance/validation branches and configuration defaults.
   - Add tests for config and common utilities (`app-config.service.ts`, `abstract-entity.service.ts`, `alias-lookup.service.ts`) to lock in mapping/merge behaviors and error handling.
...
2. Write unit tests for `publication.service.ts` and authorization/config utilities (highest impact on stability).
