...
@ExportService({path: 'cost_center'})
@Injectable()
export class CostCenterExportService extends AbstractExportService {
    ...
    protected name = 'Kostenstellen-Export';
    ...
    public async export(filter?:{filter:SearchFilter, paths:string[]}, filterServices?:AbstractFilterService<PublicationIndex|Publication>[], by_user?: string) {
        ...
        const pubs = await this.service.getCostCenters();

        let res = "id;label;number\n";
        for (const pub of pubs) {
            res+=this.format(pub.id);
            res+=this.format(pub.label);
            res+=this.format(pub.number);
