...
@ExportService({path: 'pub-type'})
@Injectable()
export class PubTypeExportService extends AbstractExportService {

    quote = '"';
    sep = ';';
    df:Intl.DateTimeFormat;

    constructor(private service:PublicationTypeService, private reportService:ReportItemService) {
        super(); 
        this.df = new Intl.DateTimeFormat('de-DE');
    }

    protected name = 'Publikationsarten-Export';
    ...
    public async export(filter?:{filter:SearchFilter, paths:string[]}, filterServices?:AbstractFilterService<PublicationIndex|Publication>[], by_user?: string) {
        ...
        let res = "id;label;reviewed\n";
        for (let pub of pubs) {
            res+=this.format(pub.id);
            res+=this.format(pub.label);
            res+=this.format(pub.review);
