...
@Component({
  selector: 'app-validation-form-log',
  imports: [SharedModule, PublicationChangeLogComponent],
  templateUrl: './validation-form-log.component.html',
  styleUrl: './validation-form-log.component.css',
})
export class ValidationFormLogComponent implements OnInit {
  workflow: ValidationWorkflow | null = null;
  ...
  ngOnInit(): void {
    this.facade.validation$
      .pipe(filter((workflow): workflow is ValidationWorkflow => workflow != null), takeUntil(this.facade.destroy$))
      .subscribe((workflow) => {
    ...
    this.route.paramMap
      .pipe(
        ...
        switchMap((params) => this.validationWorkflowService.getWorkflowReport(Number(params.get('reportId')))),
